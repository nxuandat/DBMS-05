...
--Procedure Nha sĩ thêm lịch cá nhân (LICHNHASI) có kiểm tra lỗi trùng lịch hẹn và trùng lịch cá nhân
CREATE PROCEDURE ThemLichCaNhan
  @MaNS char(20),
  @NgayBatDau datetime,
  @NgayKetThuc datetime,
  @GioBatDau datetime,
  @GioKetThuc datetime,
  @TinhTrangCuocHen char(10)
AS
BEGIN
  -- Khởi tạo biến retry
  DECLARE @retry INT;
  ...
  WHILE @retry > 0
    ...
    END;
            ...
            -- Kiểm tra xem lịch cá nhân đã tồn tại chưa
            IF EXISTS (
        SELECT 1
    FROM LICHNHASI L WITH (XLOCK,ROWLOCK)
      JOIN INSERTED I ON L.MaNS = I.MaNS
    WHERE (I.NgayBatDau BETWEEN L.NgayBatDau AND L.NgayKetThuc)
      OR (I.NgayKetThuc BETWEEN L.NgayBatDau AND L.NgayKetThuc)
      OR (I.NgayBatDau < L.NgayBatDau AND I.NgayKetThuc > L.NgayKetThuc)
      OR (I.NgayBatDau > L.NgayBatDau AND I.NgayKetThuc < L.NgayKetThuc)
    )
    BEGIN
      RAISERROR ('Lịch cá nhân đã tồn tại.', 16, 1);
      ROLLBACK TRANSACTION;
      RETURN;
    END;
            ...
            -- Thêm lịch cá nhân
            INSERT INTO LICHNHASI
      (MaNS, NgayBatDau, NgayKetThuc, GioBatDau, GioKetThuc, TinhTrangCuocHen)
    VALUES
      (@MaNS, @NgayBatDau, @NgayKetThuc, @GioBatDau, @GioKetThuc, @TinhTrangCuocHen);
...
--Procedure Nha sĩ cập nhật lịch cá nhân (LICHNHASI)
CREATE PROCEDURE UpdateLichNhaSi
  @MaNS char(20),
  @NgayBatDau datetime,
  @NgayKetThuc datetime,
  @GioBatDau datetime,
  ...
  WHILE @retry > 0
    ...
    BEGIN TRY
            ...
            SET NgayBatDau = @NgayBatDau,
                ...
                TinhTrangCuocHen = @TinhTrangCuocHen
            WHERE MaNS = @MaNS;
