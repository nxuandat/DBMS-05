...
export default function TableExpiredMedicineAdmin() {
        ...
        if (response.data && response.data.medicines) {
          ...
          setRows(
            response.data.medicines
              .filter(
                (medicine: Medicine) =>
                  new Date(medicine.NgayHetHan) < currentDate
              )
  ...
  return (
    <div>
      <h2 style={{ color: "red", margin: "30px 0" }}>
        DANH SÁCH THUỐC HẾT HẠN SỬ DỤNG CẦN XÓA
      </h2>
      <DataGrid
        rows={rows}
        columns={columns}
