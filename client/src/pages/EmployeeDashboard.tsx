...
export function EmployeeDashboard() {
    ...
    const getAppointments = async () => {
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_REACT_SERVER_PORT}/employee/get-all-appointments`,
                { withCredentials: true }
            );
        ...
        <Box className={styles.dashboardBody} component="main" sx={{ flexGrow: 1, p: 3 }}>
			<div id={styles.welcomeBanner}>
				<div className='text-white'>
                    ...
                    <br />
					Ở đây bạn có thể xem các cuộc hẹn, các hoá đơn của phòng khám và các dịch vụ mà bạn cung cấp.
					<br/>
			    </div>
			</div>
			...
			<div className={styles.statCardGridDoctor}>
                <div className={["", styles.statCard].join(" ")}>
                        <div className={styles.dashWidget}>
                            ...
                            <div className={[" ", styles.dashWidgetInfo].join(" ")} >
                                ...
                                <span className={styles.widgetTitle3}>Cuộc hẹn <i className="fa fa-check" aria-hidden="true"></i></span>
                                <hr />
                                        ...
                                        <ul>
                                            {appointmentsByDate.map(appointment => (
                                                <li key={appointment.STT}>
                                                    <strong>Mã số:</strong> {appointment.MaSoHen}<br />
                                                    <strong>Mã khách hàng:</strong> {appointment.MaKH}<br />
                                                    <strong>Ngày giờ khám:</strong> {new Date(appointment.GioBatDau).toLocaleString()}<br />
                ...
                <div className={["", styles.statCard].join(" ")}>
                    <div className={styles.dashWidget}>
                        <span className={styles.dashWidgetBg2}><i className="fa fa-file-invoice" aria-hidden="true"></i></span>
                        <div className={[" ", styles.dashWidgetInfo].join(" ")} >
                            <h3 className={styles.dashWidgetInfoH3}>{invoicesCount}</h3>
                            <span className={styles.widgetTitle2}>Hoa Don Gan Day <i className="fa fa-check" aria-hidden="true"></i></span>
                            <hr />
                            <div className={styles.dashWidgetInfoH4}>
                                <ul>
                                        {invoices.map(invoice => (
                                            <li key={invoice.MaHoaDon}>
                                                <strong>Mã dịch vụ:</strong> {invoice.MaDV}<br />
                                                <strong>Mã khách hàng:</strong> {invoice.MaKH}<br />
                                                <strong>Mã nhân viên:</strong> {invoice.MaNV}<br />
                                                <strong>SDT:</strong> {invoice.SoDT}<br />
                                                <strong>Ngày xuất:</strong> {new Date(invoice.NgayXuat).toLocaleString()}<br />
                                                <strong>Tổng Chi Phí:</strong> {invoice.TongChiPhi}<br />
                                                <strong>Tình trạng thanh toán:</strong> {invoice.TinhTrangThanhToan}<br />
